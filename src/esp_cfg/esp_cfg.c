...
{
    if (cfg_num > ESP_CFG_MAX_INDEX)
        return ESP_CFG_ERR_NOT_SUPPORTED;
    if (user_configs[cfg_num] == NULL)
        return ESP_CFG_ERR_INIT;
...
esp_cfg_init(uint8_t cfg_num, void *config, uint32 size)
{
    ...
    if (!(err == ESP_CFG_OK || err == ESP_CFG_ERR_INIT))
        return err;

    user_configs[cfg_num] = os_zalloc(sizeof(user_config));
    if (user_configs[cfg_num] == NULL)
        return ESP_CFG_ERR_MEM;
...
esp_cfg_read(uint8_t cfg_num)
{
    ...
    uint32 sec_adr = (uint32) ((ESP_CFG_START_SECTOR + cfg_num) *
                               SPI_FLASH_SEC_SIZE);
...
esp_cfg_write(uint8_t cfg_num)
{
    ...
    SpiFlashOpResult result = spi_flash_erase_sector(
        (uint16) (ESP_CFG_START_SECTOR + cfg_num));
