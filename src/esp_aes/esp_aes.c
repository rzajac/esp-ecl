...
static void InvMixColumns(void)
{
    ...
    for (i = 0; i < 4; ++i) {
        ...
        (*state)[i][0] =
            Multiply(a, 0x0e) ^ Multiply(b, 0x0b) ^ Multiply(c, 0x0d) ^
            Multiply(d, 0x09);
        (*state)[i][1] =
            Multiply(a, 0x09) ^ Multiply(b, 0x0e) ^ Multiply(c, 0x0b) ^
            Multiply(d, 0x0d);
        (*state)[i][2] =
            Multiply(a, 0x0d) ^ Multiply(b, 0x09) ^ Multiply(c, 0x0e) ^
            Multiply(d, 0x0b);
        (*state)[i][3] =
            Multiply(a, 0x0b) ^ Multiply(b, 0x0d) ^ Multiply(c, 0x09) ^
            Multiply(d, 0x0e);
    }
}
                         ...
                         const uint8_t *key, const uint8_t *iv)
{
    ...
    const uint8_t remainders = (const uint8_t) (length %
                                                KEYLEN); /* Remaining bytes in the last non-full block */
    ...
    if (remainders) {
        ...
        memset(output + remainders, 0,
               (size_t) (KEYLEN - remainders)); /* add 0-padding */
        XorWithIv(output);
        state = (state_t *) output;
        Cipher();
        ret += KEYLEN;
                         ...
                         const uint8_t *key, const uint8_t *iv)
{
    ...
    if (remainders) {
        FragmentCopy(output, input, remainders);
        os_memset(output + remainders, 0,
                  (unsigned int) (KEYLEN - remainders)); /* add 0-padding */
        state = (state_t *) output;
        InvCipher();
        ret += KEYLEN;
    }
